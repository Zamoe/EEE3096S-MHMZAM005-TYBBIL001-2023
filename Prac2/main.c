...
//static uint8_t patterns[6] = {0xAA, 0x55, 0xCC, 0x33, 0xF0, 0x0F};
static uint8_t patterns[6] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
signed int tempAddress = 0;  //used for Logic in interrupt function
signed int tempPB = 0; //used for switch case in main function
...
int main(void)
{
  ...
  //6 bitpatterns so iterate 6 times to write all 6 values
  uint16_t i = 0;
    while (i<6){
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  if ((GPIOA->IDR & GPIO_IDR_0)==0){
	  	  		//set flags according to instructions
	  		  //use tempPB as reference for current state of timer.
	  		  //tempPB=0 means current delay is 1 second
		  if (tempPB == 0){
			  htim16.Instance->ARR = 500;
	  	  	  tempPB = 1;
	  	  }
	  	  		//tempPB=1 means current delay is 500 milliseconds
	  	  else if(tempPB == 1){
...
void TIM16_IRQHandler(void)
{
	...
	if (read_from_address(tempAddress)==patterns[tempAddress]){
		//write value read from EEEprom to ODR and then clear ODR
		GPIOB-> ODR |= read_from_address(tempAddress);
		GPIOB-> ODR &= read_from_address(tempAddress);
	}
	else{
			//failsafe for failed SPI
