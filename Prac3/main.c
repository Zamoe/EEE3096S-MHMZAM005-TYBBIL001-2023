...
unsigned long debounceDuration = 200;    // time for debouncing; so far best results with 200
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	...
	raw_adc = pollADC(); //read raw ADC vale ranging 0 to 4095
	//convert uint32_t to char array using sprintf
	sprintf((char*) &buffer, "%lu",raw_adc); //%lu is for long unsigned int as uint32_t
	writeLCD((char*) &buffer);//writeLCD requires *char

	// Update PWM value; TODO: Get CRR
	CCR = ADCtoCCR(raw_adc);//pass raw_adc to ADCtoCCR
...
void EXTI0_1_IRQHandler(void)
{
	...
	if ((currentTick - debounceTicks) > debounceDuration){
		if (delay_t == 500){ 	//change frequency of LED by changing delay_t variable -> HAL_Delay (delay_t)
				delay_t = 1000; //1hz = 1000ms & 2hz = 500ms so switch between 2 of them
		}
...
void writeLCD(char *char_in){
	//char_in is char array
    delay(3000);
	lcd_command(CLEAR); //clear LCD
	lcd_putstring(char_in); //use putstring instead of putchar because used a char array to create a "string"
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	val = HAL_ADC_GetValue(&hadc); 	//get ADC value
	return val;
}
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	uint32_t val;
	//DC = CRR/ARR, so CRR = DC*ARR
	//Duty Cycle is current ADC divided by total ADC (4095)
	//ARR ranges from 0 to 47999, so CRR converted to fraction out of 47999
	val = ((float)adc_val/(float)4095) * 47999;
	return val;
}
